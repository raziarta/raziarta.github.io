...
const Game = {
  ...
  async startLevel(index) {
    ...
    if (s && s.spawnX !== undefined) {
      LevelManager.camX = s.spawnX - 200;
      LevelManager.stageDistance = LevelManager.camX;
      this.player.x = 200;
      
      // Skip waves and bosses behind the spawn point
      const threshold = LevelManager.stageDistance + Game.width;
      if (s.waves) {
        while (LevelManager.waveIndex < s.waves.length && s.waves[LevelManager.waveIndex].dist < threshold) {
          LevelManager.waveIndex++;
        }
      }
      if (s.midBoss && s.midBoss.dist < threshold) LevelManager._midBossSpawned = true;
      if (s.boss && s.boss.dist < threshold) LevelManager._bossSpawned = true;
    }
    if (s && s.spawnY !== undefined) {
      this.player.y = s.spawnY;
      this.player.lastGroundY = s.spawnY;
      if (s.trackVertical) {
        const groundY = (typeof Physics !== 'undefined' ? Physics.GROUND_Y : 800);
        LevelManager.camY = s.spawnY - groundY + 100;
      }
    }
    try {
  ...
  update(){
    ...
    if(this.state==='PLAYING'){
      ...
      if (LevelManager.isStageComplete() && !EnemyManager.midBoss && !this.boss && this.fadeState === 'none') {
        ...
        if (LevelManager.currentStage === 0 || LevelManager.currentStage === 6) {
          // Tutorial or Chapter 2 complete -> Return to world map
          this.state = 'WORLD_MAP';
          WorldMap.active = true;
          WorldMap.moving = false;
          if(typeof AudioManager !== 'undefined') AudioManager.stopAll();
          return;
        } else if (LevelManager.hasNextStage() && LevelManager.currentStage < 9) {
  ...
  draw(){
    ...
    // Main Ground (Only if no pitfalls OR within first 600 world-x)
    const hasPitfalls = stage?.hasPitfalls;
    const groundEndX = 600 - LevelManager.camX;
