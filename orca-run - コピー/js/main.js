  ...
  update(){
    ...
    if(this.state==='PLAYING'){
      ...
      if(LevelManager.isStageComplete()&&!EnemyManager.midBoss){
        if(LevelManager.currentStage === 0) {
          this.state = 'WORLD_MAP';
          WorldMap.active = true;
          WorldMap.moving = false; // Stop player movement if any
          return;
        }
        if(LevelManager.hasNextStage()){
    ...
    if(this.state==='MIDBOSS'){
      ...
      if(!EnemyManager.midBoss){
        this.state='PLAYING';
        // Stage complete (after mid-boss/boss cleared)
        if(LevelManager.isStageComplete() && !EnemyManager.midBoss && !EnemyManager.boss) {
          // Tutorial exit
          if (LevelManager.currentStage === 0) {
            this.state = 'WORLD_MAP';
            WorldMap.active = true;
          ...
          // Main Stages (1-6) sequence
          if(LevelManager.hasNextStage() && LevelManager.currentStage < 6){
            this._startFadeTransition();
          } else if (LevelManager.currentStage === 6) {
            // Chapter 2 complete, return to world map
            this.state = 'WORLD_MAP';
            WorldMap.active = true;
            WorldMap.moving = false;
            if(typeof AudioManager !== 'undefined') AudioManager.stopAll();
          } else if (LevelManager.hasNextStage() && LevelManager.currentStage >= 7 && LevelManager.currentStage < 9) {
            // Chapter 3 stages (7-9) chain
            this._startFadeTransition();
          } else {
            // Final stage cleared!
            this.state = 'ENDING';
    ...
    if(this.state==='BOSS'){
      if(this.boss&&this.boss.alive){
        ...
        if (LevelManager.hasNextStage() && LevelManager.currentStage < 6) {
          // Evolve after clearing Stage 3 (Chapter 1 climax) to prepare for Chapter 2 (Stage 4)
          if (LevelManager.currentStage === 3 && !this.player.evolved) {
            this.player.evolved = true;
          }
          this.boss = null;
          this._startFadeTransition();
        } else if (LevelManager.currentStage === 6) {
          // Chapter 2 boss cleared -> Return to World Map
          this.boss = null;
          this.state = 'WORLD_MAP';
          WorldMap.active = true;
          WorldMap.moving = false;
