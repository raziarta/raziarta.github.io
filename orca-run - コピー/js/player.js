...
class Player {
  ...
  update(keys, mouseL, mouseR, platforms, camX, screenW, screenH) {
    ...
    if (!this.knockback) {
      ...
      if (keys['ArrowLeft']||keys['a']) { this.vx = -curSpeed; this.facing = -1; }
      if (keys['ArrowRight']||keys['d']) { this.vx = curSpeed; this.facing = 1; }
    }
    ...
    if (Physics.pitCheck(this, screenH, camY)) { this.hp=0; this.alive=false; }
