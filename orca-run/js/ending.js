...
const Ending = {
  ...
  draw(ctx,w,h){
    ...
    if(this.phase>=2){
      ctx.save();
      const ox=this.orcaX, oy=h-125;
      
      // Determine walk frame at half speed (divisor 20 instead of 10)
      const walkFrame = Math.floor(this.timer / 20) % 4;
      let imgPath = 'assets/sprites/player/orca_stand.png';
      if (walkFrame === 1) imgPath = 'assets/sprites/player/orca_walk_1.png';
      else if (walkFrame === 3) imgPath = 'assets/sprites/player/orca_walk_2.png';
      
      const img = AssetLoader.images[imgPath];
      if (img) {
        ctx.filter = 'brightness(0)';
        const scale = 1.0;
        const dw = 70 * scale;
        const dh = 70 * scale;
        ctx.drawImage(img, ox - dw/2, oy - dh * 0.82, dw, dh);
        ctx.filter = 'none';
      } else {
        // Fallback to primitive if images not loaded
