...
class SignalJelly extends Enemy {
  ...
  update(player) {
    ...
    if (!player.invincible && Physics.aabb(hitProxy, player)) player.takeDamage(this.damage, this.x);
  }
...
class AbyssalChimera extends Enemy {
  constructor() {
    super(Game.width, 'abyssal_chimera', 3.0);
    ...
    this.hitStop = 0;
    this.attackPhase = 0;
    this.name = 'ABYSSAL CHIMERA';
  }
  update(player) {
    this.frame++; this.stateTimer--;
    
    // Smooth floating
    this.y = this.baseY + Math.sin(this.frame * 0.05) * 60;

    // Bounds checking to stay on screen
    const margin = 80;
    this.x = Math.max(margin, Math.min(Game.width - margin, this.x));
    this.baseY = Math.max(100, Math.min(Game.height - 200, this.baseY));
    
    switch (this.state) {
      case 'enter':
        this.x -= 2.5; 
        if (this.x <= 600) { this.state = 'attack'; this.stateTimer = 120; }
        break;
      case 'attack':
        ...
        if (this.attackPhase === 0 && this.frame % 30 === 0) {
          // Bioluminescent homing orbs
          ProjectileManager.addEnemyBullet(this.x, this.y, -4, Math.sin(this.frame)*2, 'energy');
        } else if (this.attackPhase === 1 && this.frame % 20 === 0) {
          // Scatter shot
          for(let i=0; i<3; i++) {
             const angle = Math.PI + (i-1)*0.3;
             ProjectileManager.addEnemyBullet(this.x, this.y, Math.cos(angle)*6, Math.sin(angle)*6, 'energy');
          }
        }
        break;
      case 'dash':
      ...
      case 'return':
        ...
        this.baseY += (200 - this.baseY) * 0.05;
        if (Math.abs(this.x - 600) < 5) {
          this.state = 'attack';
          this.stateTimer = 180;
        }
        break;
  ...
  die() {
    ...
    if (typeof ItemManager !== 'undefined') {
      ItemManager.spawn(this.x, this.y, 'healM'); // 100% drop for boss
    }
    if (typeof AudioManager !== 'undefined') AudioManager.playSE('enemyDie');
  }
  draw(ctx) { Renderer.drawAbyssalChimera(ctx, this, this.frame); }
}
...
class AbyssAngler extends Enemy {
  ...
  takeDamage(d) { 
    this.hp -= d; 
    if (typeof ParticleSystem !== 'undefined') ParticleSystem.burst(this.x, this.y, '#44aaff', 6); 
    if (this.hp <= 0) this.die(); 
  }
  die() {
    this.alive = false;
    if (typeof ItemManager !== 'undefined') {
      ItemManager.spawn(this.x, this.y, 'healM');
    }
    if (typeof AudioManager !== 'undefined') AudioManager.playSE('enemyDie');
  }
  draw(ctx) {
...
EnemyRegistry.register('abyssal_chimera', AbyssalChimera);
