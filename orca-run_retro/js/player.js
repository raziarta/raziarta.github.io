...
class Player {
  ...
  update(keys, mouseL, mouseR, platforms, camX, screenW, screenH) {
    ...
    // Shoot normal (left click or Z) - allowed during crouch for low enemies
    if ((mouseL || keys['z'] || keys['Z']) && this.shotCooldown <= 0) {
      this.shootNormal();
      this.shotCooldown = this.shotCooldownMax;
    }

    // Shoot bomb (right click or X) - allowed during crouch
    if ((mouseR || keys['x'] || keys['X']) && this.bombCooldown <= 0) {
      this.shootBomb();
  ...
  shootNormal() {
    ...
    const shotY = this.crouching ? this.y + 2 : this.y - 27;
    ProjectileManager.addPlayerBullet(this.x + this.facing * 40, shotY, this.facing, 'normal', finalMul * (baseDmg/10), this.evolved);
  ...
  shootBomb() {
    ...
    const shotY = this.crouching ? this.y + 2 : this.y - 30;
    ProjectileManager.addPlayerBullet(this.x + this.facing * 40, shotY, this.facing, 'bomb', finalMul * (baseDmg/50), this.evolved);
