...
class PenguinBomb {
  ...
  update() {
    ...
    const camX = (typeof LevelManager !== 'undefined') ? LevelManager.camX : 0;
    const worldX = this.x + camX;
    const groundY = (typeof LevelManager !== 'undefined') ? LevelManager.camY + Physics.GROUND_Y : Physics.GROUND_Y;
    const hasPitfalls = (typeof LevelManager !== 'undefined' && LevelManager.getCurrentStage()?.hasPitfalls);

    if (!hasPitfalls || worldX < 600) {
      if (this.y + this.hh >= groundY) {
        this.y = groundY - this.hh;
        hitSurface = true;
      }
    }

    if (!hitSurface && typeof LevelManager !== 'undefined') {
      const plats = LevelManager.platforms;
      for (const p of plats) {
        if (this.vy >= 0 && worldX + this.hw > p.x && worldX - this.hw < p.x + p.w &&
            this.y + this.hh >= p.y && this.y + this.hh - this.vy <= p.y + 15) {
          this.y = p.y - this.hh;
          hitSurface = true;
          break;
        }
