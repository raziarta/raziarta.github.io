...
const Renderer = {
  ...
  drawAbyssalChimera(ctx, boss, frame) {
    const { x, y, hp, maxHp, name } = boss;
    const scale = 1.0;
    ctx.save(); ctx.translate(x, y);
    // Main body
    ctx.fillStyle = '#112233';
    ctx.beginPath(); ctx.ellipse(0, 0, 20*scale, 15*scale, 0, 0, Math.PI*2); ctx.fill();
    // Glowing spots
    ctx.fillStyle = `hsl(${(frame*4)%360}, 100%, 50%)`;
    ctx.beginPath(); ctx.arc(-10*scale, -5*scale, 3*scale, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(10*scale, 5*scale, 3*scale, 0, Math.PI*2); ctx.fill();
    // Tentacles
    ...
    for(let i=0; i<3; i++) {
       ctx.beginPath();
       ctx.moveTo(-15*scale + i*15*scale, 10*scale);
       ctx.quadraticCurveTo(-15*scale + i*15*scale + Math.sin(frame*0.1+i)*10*scale, 30*scale, -15*scale + i*15*scale + Math.sin(frame*0.1+i+1)*20*scale, 40*scale);
       ctx.stroke();
    }
    ctx.restore();
    
    // Draw Boss UI
    this.drawBossHealthBar(ctx, hp, maxHp, name);
  },

  drawBossHealthBar(ctx, hp, maxHp, name) {
    const W = 800; // Standard logical width
    const barW = 400;
    const barH = 12;
    const x = (W - barW) / 2;
    const y = 40;
    ...
    ctx.fillStyle = ratio > 0.5 ? '#00ffcc' : (ratio > 0.25 ? '#ffcc00' : '#ff4444');
