    ...
    if (type === 0) {
        ...
        const fire = (dx, dy, dz) => {
            ...
            if (G.isHost || !G.isOnline) {
                fireProjectile(spawnX, spawnY, spawnZ, vx, vy, vz, G.playerBody);
            } else {
        ...
        const fireB = (ox, oz) => {
            if (G.isHost || !G.isOnline) {
                createBubble(bx + ox, by, bz + oz, vx, vy, vz, G.playerBody);
            } else {
                broadcastEvent(10, { type: 1, x: bx + ox, y: by, z: bz + oz, vx, vy, vz, ownerName });
            }
        };
...
function spawnProjectileMesh(netId, x, y, z) {
    ...
    const rMult = config.projectileRadiusMult || 1.0;
    mesh.scale.set(rMult, rMult, rMult);
