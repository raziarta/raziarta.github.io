                    ...
                    if (!(other.belongsTo & 1) && other !== b.ownerBody) {
                        ...
                        G.entities.forEach(ent => {
                            if (ent.isAI && ent.body === other) {
                                ...
                                takeDamage(b.props ? b.props.damage : config.damageBubble);
                            }
                        });
                        ...
                        if (G.isHost && G.isOnline) {
                            for (const [peerId, netEnt] of G.networkEntities) {
                                if (netEnt.body === other) {
                                    ...
                                    // ノックバック計算はupdateSoapBubblesで行われるためここではEvent 21でダメージ通知
                                    // （ノックバックはcombat.jsのupdateSoapBubbles内でsendToClientされる）
                                    break;
                                }
                            }
                        }
    ...
    if (G.isStarted && G.playerLives>0) {
        G.projectiles.forEach(p => {
            if (p.ownerBody!==G.playerBody && !p._hitFlag) {
                ...
                if (dx*dx+dy*dy+dz*dz<0.35) {
                    ...
                    if (G.isHost || !G.isOnline) {
                        takeDamage(p.props ? p.props.damage : config.damageProjectile);
                    }
                }
            }
        });
        G.bubbles.forEach(b => {
            ...
            if (!isMyBubble&&!b._hitFlag&&(Date.now()-(b.spawnTime||0))>300) {
                const dx=bPos.x-G.playerBody.position.x,dy=bPos.y-G.playerBody.position.y,dz=bPos.z-G.playerBody.position.z;
