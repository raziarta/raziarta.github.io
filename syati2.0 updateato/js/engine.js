...
function animate() {
                ...
                if (link.contact && link.contact.touching) {
                    ...
                    if (!(other.belongsTo & 1) && other !== b.ownerBody) {
                        ...
                        G.entities.forEach(ent => {
                            ...
                            } else if (other === G.playerBody) { takeDamage(config.damageBubble); }
                        });
                        break;
                    }
                }
    ...
    if (Math.abs(vel.x) > 0.1 || Math.abs(vel.z) > 0.1) {
        ...
        G.playerMesh.quaternion.slerp(new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0,1,0), ta), 0.15);
    }
    ...
    const isRewarding = !document.getElementById('reward-screen').classList.contains('hidden');

    if (!isRewarding) handleJump(G.keys.space);
    if (G.isGrounded) { G.jumpCount = 0; G.isJumping = false; }

    const fwd = new THREE.Vector3(0,0,-1).applyQuaternion(G.camera.quaternion); fwd.y=0; fwd.normalize();
    ...
    const offset = new THREE.Vector3(0,0.97,G.camDist); offset.applyQuaternion(G.camera.quaternion);
    G.camera.position.set(pos.x+offset.x, pos.y+offset.y, pos.z+offset.z);
    ...
    // フォグ
    if (G.camera.position.y<-0.51) { G.scene.fog.color.set(0x0055aa); G.scene.fog.near=0; G.scene.fog.far=40; }
    else { G.scene.fog.color.set(0x87CEEB); G.scene.fog.near=Math.max(1,pos.y+20); G.scene.fog.far=pos.y+400; }
    ...
    if (G.isStarted && !G.isGoalReached && G.currentMode === 'main' && pos.y >= config.goalHeight) {
        ...
        saveRecord(config.goalHeight, elapsed);
        // alertを削除
    }
    ...
    G.networkEntities.forEach(ent => {
        if (ent.targetNetPos) {
            const dp=ent.targetNetPos.clone(); dp.y-=0.37;
            const dx=dp.x-ent.mesh.position.x, dz=dp.z-ent.mesh.position.z;
            if (Math.abs(dx)>0.01||Math.abs(dz)>0.01) ent.mesh.quaternion.slerp(new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0,1,0),Math.atan2(dx,dz)),0.15);
            ent.mesh.position.lerp(dp,0.2);
        }
    });

    // 弾丸自動発射
    if (G.isStarted && config.projectileAutoFire && G.controls && G.controls.isLocked && isRewarding === false) {
    ...
    if (G.isStarted && G.playerLives>0) {
        G.projectiles.forEach(p => {
            if (p.ownerBody!==G.playerBody && !p._hitFlag) {
                ...
                if (dx*dx+dy*dy+dz*dz<0.35) { p._hitFlag=true; takeDamage(config.damageProjectile); }
            }
        });
        G.bubbles.forEach(b => {
            if (!b.body) return;
            ...
            if (!isMyBubble&&!b._hitFlag&&(Date.now()-(b.spawnTime||0))>300) {
                ...
                if (dx*dx+dy*dy+dz*dz<8.2) { b._hitFlag=true; takeDamage(config.damageBubble); }
            }
        });
    }
    updateSoapBubbles();
    ...
    if (G.isHost&&G.isStarted) {
        ...
        if (window.syncCounter%2===0) {
            const sl=[];
            G.projectiles.forEach(p=>{if(p.netId) sl.push({id:p.netId,x:p.position.x,y:p.position.y,z:p.position.z});});
            G.bubbles.forEach(b=>{if(b.netId) sl.push({id:b.netId,x:b.body.position.x,y:b.body.position.y,z:b.body.position.z});});
            if (sl.length>0) broadcastEvent(12,{list:sl});
        }
    }

    if (G.isOnline) broadcastEvent(1,{id:G.myPeerId,x:G.playerBody.position.x,y:G.playerBody.position.y,z:G.playerBody.position.z,jumps:G.jumpCount});
    ...
    if (G.isStarted&&window.birds) {
        ...
        for (let i=window.birds.length-1;i>=0;i--) {
            ...
            b.wingPhase+=15*dt; b.wingL.rotation.y=Math.sin(b.wingPhase)*0.5; b.wingR.rotation.y=-Math.sin(b.wingPhase)*0.5;
