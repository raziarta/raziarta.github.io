        ...
        if (!useNN) {
            ...
            if (needNewTarget) {
                ...
                if (followPlayer && G.playerBody) {
                    ...
                    const getBestTarget = (radiusH, radiusUp, radiusDown) => {
                        const candidates = findLandableBlocks(pos, radiusH, radiusUp, radiusDown);
                    ...
                    // 1回目の探索（通常範囲）
                    let bestTarget = getBestTarget(8, 6, 2);

                    // 見つからない場合は索敵範囲を広げて再探索
                    if (!bestTarget) {
                        bestTarget = getBestTarget(15, 12, 5);
                    }
                    ...
                    if (bestTarget) {
                        ...
                        // ターゲットが見つからない場合は「探し回る」（ランダム方向へのアプローチ）
                        ent.targetPos = new THREE.Vector3(
            ...
            if (ent.targetPos) {
                ...
                // ターゲットへの理想的なジャンプ距離（高低差に応じて遠くから跳ぶ）
                const jumpDist = targetIsUp ? Math.max(0.5, Math.min(3.5, dyToTarget * 1.5)) : 1.0;

                const shouldJump =
                    (targetIsUp && ent.isGrounded && distH <= jumpDist) // ターゲットが上で、十分近づいた時のみジャンプ
                    || (targetIsUp && !ent.isGrounded && ent.jumpCount === 1 && vel.y < 0.5) // 空中で落下し始めたら2段ジャンプを消費して飛距離を稼ぐ
