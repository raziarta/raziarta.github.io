...
function init() {
    ...
    G.scene.fog = new THREE.Fog(0x87CEEB, 50, 400);
    ...
    const seaMat = new THREE.MeshStandardMaterial({ color: 0x0077be, transparent: true, opacity: 0.6, metalness: 0.4, roughness: 0.1, side: THREE.DoubleSide });
    ...
    G.scene.fog.color.set(0x87CEEB);
    ...
    window.createBirdMesh = function() {
        ...
        return { group, wingL: group.getObjectByName("WingL"), wingR: group.getObjectByName("WingR") };
    };
    window.spawnBirds = function() {
        ...
        for (let i=0; i<count; i++) {
            ...
            G.scene.add(b.group);
            window.birds.push({ mesh: b.group, wingL: b.wingL, wingR: b.wingR, dir: new THREE.Vector3(dirX, 0, dirZ).normalize(), speed: 12 + Math.random() * 4, wingPhase: Math.random() * Math.PI * 2, distance: 0 });
        }
    };
