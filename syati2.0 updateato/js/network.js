...
function packData(eventId, data) {
    ...
    if (eventId === 10) {
        // [10, type, x, y, z, vx, vy, vz, [radiusMult, passWall, bounces, rangeMult, damage, isNeedle, speed]]
        const p = data.props;
        const packedProps = p ? [p.radiusMult, p.passWall?1:0, p.bounces, p.rangeMult, p.damage, p.isNeedle?1:0, p.speed] : null;
        return [10, data.type, 
    ...
    if (eventId === 11) {
        // [11, netId, type, x, y, z, vx, vy, vz, [props]]
        const p = data.props;
        const packedProps = p ? [p.radiusMult, p.passWall?1:0, p.bounces, p.rangeMult, p.damage, p.isNeedle?1:0, p.speed] : null;
...
function unpackData(arr) {
    ...
    if (Array.isArray(arr)) {
        ...
        if (ev === 10) {
            const pp = arr[8];
            const props = pp ? { radiusMult: pp[0], passWall: !!pp[1], bounces: pp[2], rangeMult: pp[3], damage: pp[4], isNeedle: !!pp[5], speed: pp[6] } : null;
            return { event: 10, type: arr[1], x: arr[2], y: arr[3], z: arr[4], vx: arr[5], vy: arr[6], vz: arr[7], props };
        }
        if (ev === 11) {
            const pp = arr[9];
            const props = pp ? { radiusMult: pp[0], passWall: !!pp[1], bounces: pp[2], rangeMult: pp[3], damage: pp[4], isNeedle: !!pp[5], speed: pp[6] } : null;
            return { event: 11, netId: arr[1], type: arr[2], x: arr[3], y: arr[4], z: arr[5], vx: arr[6], vy: arr[7], vz: arr[8], props };
        }
    ...
    if (data.event === 0) {
        ...
        if (G.isHost) {
            ...
            G.projectiles.forEach(p => {
                if (p.netId) broadcastEvent(11, { netId: p.netId, type: 0, x: p.position.x, y: p.position.y, z: p.position.z, vx: p.velocity.x, vy: p.velocity.y, vz: p.velocity.z, props: p.props });
            });
            G.bubbles.forEach(b => {
                if (b.netId) broadcastEvent(11, { netId: b.netId, type: 1, x: b.body.position.x, y: b.body.position.y, z: b.body.position.z, vx: b.body.linearVelocity.x, vy: b.body.linearVelocity.y, vz: b.body.linearVelocity.z, props: b.props });
            });
        }
